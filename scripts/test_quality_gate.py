...
class QualityGate(unittest.TestCase):
 ...
 def test_deploy_approval_governance_in_scope(self):
  self.assertTrue(all(q.in_scope(x) for x in ('scripts/deploy_with_approval.py','scripts/test_deploy_with_approval.py','scripts/test_deploy_verify_fixture.py','scripts/fixtures/approval/approval-ok.json')));self.assertFalse(q.in_scope('scripts/fixtures/other.json'));self.assertFalse(q.in_scope('scripts/fixtures/approval/new-unlisted.json'))
